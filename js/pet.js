(function () {
  ...
  function readSize() {
    var s = parseInt(localStorage.getItem("curl-pet-size"), 10);
  ...
  var COLOR_COUNT = 6;
  var petColor = 0;
  try { petColor = parseInt(localStorage.getItem("curl-pet-color"), 10) || 0; }
  ...
  function cyclePetColor() {
    ...
    applyPetColor();
    try {
      if (petColor) localStorage.setItem("curl-pet-color", String(petColor));
      else localStorage.removeItem("curl-pet-color");
    } catch (e) { /* private mode */ }
  }
  ...
  function applyOpacity() {
    var o = parseInt(localStorage.getItem("curl-pet-opacity"), 10);
    if (!(o >= 15 && o <= 100)) o = 70;
    document.documentElement.style.setProperty("--pet-base-opacity", (o / 100).toFixed(3));
  }
  function readCfg() {
    cfgNap = boolKey("curl-pet-nap", true);
    cfgFlee = boolKey("curl-pet-flee", true);
    cfgRead = boolKey("curl-pet-read", true);
    cfgTricks = boolKey("curl-pet-tricks", true);
    cfgSpeech = boolKey("curl-pet-speech", false);
    var c = parseInt(localStorage.getItem("curl-pet-color"), 10);
