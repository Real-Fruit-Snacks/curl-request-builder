...
(function () {
  ...
  function fire(key, val) {
    ...
    if (String(key).indexOf("curl-pet") === 0) window.dispatchEvent(new Event("tw:pet"));
  }
  ...
  panel.innerHTML =
    ...
    settingRow("THEME", '<span class="seg" data-pref="curl-theme">' +
      '<button data-val="dark">DARK</button><button data-val="light">LIGHT</button>' +
      '<button data-val="system">SYSTEM</button></span>') +
    settingRow("ACCENT", '<span class="seg accent-seg" data-pref="curl-accent">' +
    ...
    settingRow("MODE", '<span class="seg" data-petpref="curl-pet">' +
    ...
    settingRow("NAP", petToggle("curl-pet-nap")) +
    settingRow("FLEE", petToggle("curl-pet-flee")) +
    settingRow("READ", petToggle("curl-pet-read")) +
    settingRow("TRICKS", petToggle("curl-pet-tricks")) +
    settingRow("SPEECH", petToggle("curl-pet-speech")) +
    "</div></div>";
  ...
  function petDefaults(key) {
    return { "curl-pet": "float", "curl-pet-nap": "on", "curl-pet-flee": "on",
      "curl-pet-read": "on", "curl-pet-tricks": "on", "curl-pet-speech": "off" }[key];
  }
  ...
  function refreshSeg() {
    ...
    for (var i = 0; i < segs.length; i++) {
      ...
      var cur;
      if (key === "curl-theme") cur = TW.prefs.get("curl-theme", "system", ["dark", "light", "system"]);
      else cur = TW.prefs.get("curl-accent", "0", ["1", "2", "3", "4", "5"]);
  ...
  function refreshPet() {
    ...
    document.getElementById("pet-size").value = TW.prefs.get("curl-pet-size", "28");
    document.getElementById("pet-opacity").value = TW.prefs.get("curl-pet-opacity", "70");
  }
  ...
  panel.addEventListener("click", function (e) {
    ...
    if (key === "curl-theme") {
      if (val === "system") { TW.prefs.remove("curl-theme"); }
      else { TW.prefs.set("curl-theme", val); }
      applyTheme(val === "system" ? null : val);
      refreshSeg();
      return;
    }
    if (key === "curl-accent") {
      if (val === "0") { TW.prefs.remove("curl-accent"); }
      else { TW.prefs.set("curl-accent", val); }
